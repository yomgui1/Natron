...
# OpenImageIO
cp "$SDK/lib/libOpenImageIO.so.1.8" "$SDK/lib/libOpenImageIO_Util.so.1.8" "$D/lib/"
...
if [[ $QT4 == 1 ]]; then
	PYSIDE_DEPENDS=$(ldd $(find "$SDK/qt4/lib/python${PYVER}/site-packages/PySide${PYSIDEVER}" -maxdepth 1 -type f -name "*.so") | grep /opt | awk '{print $3}' | sort -u)
	PYSIDE_DEPENDS+=$(ldd "$SDK/qt4/lib/python${PYVER}/site-packages/shiboken.so" | grep /opt | awk '{print $3}' | sort -u)
else
	PYSIDE_DEPENDS=$(ldd $(find "$SDK/lib/python${PYVER}/site-packages/PySide${PYSIDEVER}" -maxdepth 1 -type f -name "lib*.so*") | grep /opt | awk '{print $3}' | sort -u)
	PYSIDE_DEPENDS+=$(ldd $(find "$SDK/lib/python${PYVER}/site-packages/shiboken${PYSIDEVER}" -maxdepth 1 -type f -name "lib*.so*") | grep /opt | awk '{print $3}' | sort -u)
fi
